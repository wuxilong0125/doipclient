      ...
      if (re != 0) {
        ...
        switch (doip_tcp_message.payload_type_) {
          ...
          case DoIpPayload::kDiagnosticAck: {
            ...
            gate_way->SetDiagnosticAck(true);
            // diagnostic_msg_ack = true;
            gate_way->gateway_reply_cv_.notify_all();
            break;
          }
          case DoIpPayload::kDiagnosticNack: {
            gate_way->SetDiagnosticNack(true);
          ...
          case DoIpPayload::kDiagnosticMessage: {
            ...
            gate_way->SetDiagnosticResponse(true);
...
GateWayReplyCode DoIpClient::SendECUMeassage(std::shared_ptr<GateWay> gate_way, ByteVector uds) {
  
  uds_.assign(uds.begin(), uds.end());
  CPRINT("发送");
  if (gateway_thread_map_.find(gate_way->gate_way_address_) == gateway_thread_map_.end()) {
    std::thread send_gateway_msg_thread(&DoIpClient::SendDiagnosticMessageThread, this,
                                      gate_way);
    gateway_thread_map_.insert(
        {gate_way->gate_way_address_, send_gateway_msg_thread.native_handle()});
    send_gateway_msg_thread.detach();
  } else {
    // ecu->SetUds(uds);
    // 唤醒SendDiagnosticMessage
    gate_way->SetSendAgain(true);
    gate_way->send_again_cv_.notify_all();
    CPRINT("Send Again");
    std::cout << "Before sendagain: " <<gate_way->GetSendAgain() << std::endl;
  }
  std::unique_lock<std::mutex> lock(gate_way->reply_Status_mutex_);
  // TODO: 计时
  gate_way->gateway_reply_status_cv_.wait_for(lock, std::chrono::seconds(time_diagnostic_msg_));

  return gateway_reply_status_map_[gate_way->gate_way_address_];
}

void DoIpClient::SendDiagnosticMessageThread(std::shared_ptr<GateWay> gate_way) {
  
  std::unique_lock<std::mutex> lock(gate_way->thread_mutex_);
  while(true) {
    // TODO: BUG GetSendAgain
    gate_way->send_again_cv_.wait(lock, [gate_way]{  std::cout << " sendagain: " <<gate_way->GetSendAgain() << std::endl; return gate_way->GetSendAgain();});
    std::cout << "线程" << std::endl;
    SendDiagnosticMessage(gate_way, uds_);
    gate_way->SetSendAgain(false);
  }
}

void DoIpClient::SendDiagnosticMessage(std::shared_ptr<GateWay> gate_way, ByteVector user_data) {
  std::unique_lock<std::mutex> lock(gate_way->write_mutex_);
  if (user_data.size() == 0) {
    CPRINT("Payload is NULL.");
    gateway_reply_status_map_.insert({gate_way->gate_way_address_, GateWayReplyCode::kSendError});
    gate_way->gateway_reply_status_cv_.notify_all();
    return ;
  }
  if (gate_way->tcp_socket_ <= 0) {
    CPRINT("TCP socket is error.");
  ...
  diagnostic_msg.ConstructDiagnosticMessage(source_address_, gate_way->gate_way_address_,
                                            user_data);
  if (gate_way->GetDiagnosticAck()) {
    gate_way->SetDiagnosticAck(false);
  }
  if (gate_way->GetDiagnosticNack()) {
    gate_way->SetDiagnosticNack(false);
  }
  if (gate_way->GetDiagnosticResponse()) {
    gate_way->SetDiagnosticResponse(false);
  }
  // timer->StartOnce(timeout);
  // TODO: 修改了&gateway->vehicle_ip_
  int re = SocketWrite(gate_way->tcp_socket_, diagnostic_msg, &gate_way->vehicle_ip_);
  if (-1 == re) {
    gateway_reply_status_map_.insert({gate_way->gate_way_address_, GateWayReplyCode::kSendError});
    gate_way->gateway_reply_status_cv_.notify_all();
    return ;
  ...
  gate_way->gateway_reply_cv_.wait_for(lock, std::chrono::seconds(time_diagnostic_msg_), 
                                              [&gate_way]{ return (gate_way->GetDiagnosticAck() 
                                                              || gate_way->GetDiagnosticNack()
                                                              || gate_way->GetDiagnosticResponse());});

  if (gate_way->GetDiagnosticAck()) {
    gateway_reply_status_map_.insert({gate_way->gate_way_address_, GateWayReplyCode::kACK});
    gate_way->SetDiagnosticAck(false);
    gate_way->gateway_reply_status_cv_.notify_all();
  }else if (gate_way->GetDiagnosticNack()) {
    gateway_reply_status_map_.insert({gate_way->gate_way_address_, GateWayReplyCode::kNACK});
    gate_way->SetDiagnosticNack(false);
    gate_way->gateway_reply_status_cv_.notify_all();
  }else if (gate_way->GetDiagnosticResponse()) {
    gateway_reply_status_map_.insert({gate_way->gate_way_address_, GateWayReplyCode::kResp});
    gate_way->SetDiagnosticNack(false);
    gate_way->gateway_reply_status_cv_.notify_all();
  }else {
    CPRINT("DiagnosticMessage ACK timeout.");
    gateway_reply_status_map_.insert({gate_way->gate_way_address_, GateWayReplyCode::kTimeOut});
    gate_way->gateway_reply_status_cv_.notify_all();
    return ;
  }
}
